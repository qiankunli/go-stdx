...
Go stdlib 扩展库，长期对标 Java 里 Guava 的位置——项目手写 helper 之前先来这里找。与现有生态分工：samber/lo 占泛型集合、gods 占数据结构、lancet 是 kitchen-sink；go-stdx 拼的是纪律而非广度。**名字即收录纪律**：stdlib 没有的才进（手搓 max、复刻 slices.Clone、包一层 strconv 都不属于这里）。永远零依赖；stdlib 补齐等价物后这里的条目废弃删除；**三次法则**——真实项目里重复手写 ≥3 次才配一个槽位，不预铸。
